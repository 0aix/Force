...
namespace Jotunheimr
{
		...
		if (!CreateDirectory("temp", NULL)) //Check if directory already exists
		{
			...
			if (hFind != INVALID_HANDLE_VALUE)
			{
				do
				{
					...
					{
						sprintf_s(buffer, MAX_PATH, "temp\\%s", ffd.cFileName);
						DeleteFile(buffer);
					}
	...
	bool LoadTO(int type, char* filename)
	{
		...
		TOFile[type].cName = new char[length]; //Null-terminator
		strcpy_s(TOFile[type].cName, length, filename);
	...
	void UnloadResource(int type, int ID)
	{
		...
		if (res->loc && res->state == 2) //Loaded
		{
			...
			{
				AnimPack* pack = (AnimPack*)res->loc;
				Frame* frame = pack->frame;
			...
			while (!bExit)
			{
				...
				if (pItem != NULL)
				{
					...
					if (pView != NULL)
					{
						...
						{
							BYTE count = *base;
							AnimPack* pack = new AnimPack(count, *(base + 1)); //The dereference is for loops
							DWORD index = 2; //Offset for frames
							for (BYTE i = 0; i < count; i++)
							{
								WORD v = *(WORD*)(base + index);
								LoadResource(TO::SPRITE, v, NULL);
								pack->obj[i] = &ResList[TO::SPRITE][v];
								pack->frame[i].ID = v;
								pack->frame[i].x = *(WORD*)(base + index + 2);
								pack->frame[i].y = *(WORD*)(base + index + 4);
								pack->frame[i].delay = *(WORD*)(base + index + 6);
								index += 8;
							}
		...
		try
		{
			while (!bExit)
			{
				...
				for (; pPackage != NULL; Sleep(1))
				{
					...
					{
						AnimPack* pack = (AnimPack*)pPackage->pack;
						obj = pack->obj;
						count = pack->count;
					}
			...
			while (!bExit)
			{
				...
				if (pPackage != NULL)
				{
					...
					if (pPackage->type == TO::ANIM)
					{
						...
						Frame* frame = pack->frame;
						BYTE count = pack->count;
						bool loop = pack->loops > 0; //For now, either loops or doesn't
